...
export default async function decorate(block) {
  ...
  // Utilities
  const waitForYotpoReady = (timeoutMs = 8000, intervalMs = 150) => new Promise((resolve) => {
    const start = Date.now();
    const check = () => {
      if (window.yotpo && typeof window.yotpo.refreshWidgets === 'function') return resolve(true);
  ...
  const scheduleClassicRefresh = (reason = "unspecified") => {
    ...
    if (window.yotpo && typeof window.yotpo.refreshWidgets === "function") {
      ...
      waitForYotpoReady().then((ready) => { if (ready) enqueue(); });
    }
  };
  ...
  const extractSkuFromHref = (href) => {
    try {
      ...
      const qpSku = url.searchParams.get('sku') || url.searchParams.get('productId') || url.searchParams.get('id');
  ...
  const getProductId = (anchor, tile) => {
    ...
    const fromData =
      tile?.dataset?.sku || tile?.dataset?.productSku || tile?.dataset?.productId ||
      anchor?.dataset?.sku || anchor?.dataset?.productId;
  ...
  if (!target) target = tile.querySelector('.ds-sdk-product-card__info, .product-info, .product-content');
  ...
  const injectForAnchors = (anchors, instanceId, status, variant = "classic") => {
    if (status === "off") {
  ...
  if (!tile) {
      ...
      const existingMatch = (atc ? atc.parentElement : placement)?.querySelector(`.yotpo.bottomLine[data-product-id="${sku}"]`) || placement.querySelector(`.yotpo.bottomLine[data-product-id="${sku}"]`);
      if (existingMatch) {
        ...
        if (atc && atc.parentElement && existingMatch !== atc && existingMatch.nextElementSibling !== atc) {
          try {
      ...
      const staleNodes = tile.querySelectorAll('.yotpo.bottomLine, .yotpo-widget-instance');
      if (staleNodes.length) {
        
        staleNodes.forEach((n) => n.remove());
      }
      // Create appropriate widget element
      const el = variant === "classic"
    ...
    const hasAnyBottomLines = document.querySelector('.yotpo.bottomLine') !== null;
    if (variant === "classic") {
  ...
  const collectProductAnchors = (root = document) => {
    ...
    const primary = Array.from(document.querySelectorAll('.ds-sdk-product-item a[href]'));
    if (primary.length) {
      
      return primary;
    }
    const alt = Array.from(document.querySelectorAll('.ds-sdk-product-list a[href], .ds-widgets_results a[href]'));
    if (alt.length) {
      
      return alt;
    }
    const fallback = Array.from(document.querySelectorAll('a[href*="/products/"]'));
  ...
  const enableObserver = (instanceId, status, variant = "classic") => {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      for (const m of mutations) {
        if (m.addedNodes && m.addedNodes.length) { needsProcess = true; break; }
      }
  ...
  try {
    ...
    if (!resp.ok) throw new Error(`config.endpoint response: ${resp.statusText}`);
    ...
    const isPLP = !!document.querySelector('.ds-sdk-product-item') || !!document.querySelector('.ds-widgets_results') || !!document.querySelector('.product-list-page-container');
    ...
    if (plpAnchors.length) {
      ...
      const pdpAttrs = [
        ...
        { attr: "data-yotpo-url", value: window.location.toString() },
        {
          ...
          value: `https:${
            document.querySelector(".pdp-carousel__slide>img")?.getAttribute("src") || ""
          }`,
        },
        {
